...
cp "$apktool_jar" "$output/apktool.jar"
cp "$build_tools/aapt2" "$output/aapt2"
cp "$build_tools/zipalign" "$output/zipalign"
cp "$build_tools/lib/apksigner.jar" "$output/apksigner.jar"
cp "$sdk/platform-tools/adb" "$output/adb"
chmod +x "$output/runtime/bin/java" "$output/runtime/bin/keytool" "$output/aapt2" "$output/zipalign" "$output/adb"

"$output/runtime/bin/java" -version
"$output/runtime/bin/keytool" -help >/dev/null
"$output/runtime/bin/java" -jar "$output/apktool.jar" --version
"$output/runtime/bin/java" -jar "$output/apksigner.jar" version
